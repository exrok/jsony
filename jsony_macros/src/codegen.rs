...
fn enum_from_binary(out: &mut RustWriter, ctx: &Ctx, variants: &[EnumVariant]) {
    let fallback = variants
        .iter()
        .find(|variant| match variant.kind {
            EnumKind::None => true,
            _ => false,
        })
