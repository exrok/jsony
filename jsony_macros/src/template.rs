...
impl Codegen {
    ...
    fn dyn_key(&mut self, _span: Span, expr: TokenStream) {
        self.flush_text();
        {
            (self.out).blit(903, 26);
            {
                let at = (self.out).buf.len();
                (self.out).blit_punct(5);
