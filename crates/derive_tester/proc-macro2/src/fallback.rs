...
fn push_token_from_proc_macro(mut vec: RcVecMut<TokenTree>, token: TokenTree) {
    ...
    fn push_negative_literal(mut vec: RcVecMut<TokenTree>, mut literal: Literal) {
        ...
        vec.push(TokenTree::Literal(literal));
    }
}
...
impl Ident {
    ...
    pub fn owned(string: impl Into<Box<str>>) -> Self {
        Ident::Owned(string.into())
    }

    pub fn new_raw(string: &str) -> Self {
        Ident::Raw(string.into())
    }
}
...
impl Display for Ident {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Ident::Static(s) => Display::fmt(s, f),
            Ident::Raw(s) => {
                f.write_str("r#")?;
                Display::fmt(s, f)
            }
            Ident::Owned(s) => Display::fmt(s, f),
        }
    }
}
