//! Crate-root error and flatten types, shared by the proc-macro binary
//! (`main.rs`) and the in-process driving library (`lib.rs`).
//!
//! These live outside `codegen`/`template` so `cargo expand --bin` (the macro
//! preprocessor's input) keeps them at the crate root rather than folding them
//! into a transformed module.
